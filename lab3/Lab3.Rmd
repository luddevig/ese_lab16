...
## 2. Supposing the observations come from a normal population and then define a model to generate the data.
...
## 3. Creating a data frame of population for each goup.
...
## 4. Creating a balanced sample for each group with 10 teams in each group.
...
## 5. Genrating the linear model to fit the data.

```{r 5}
m1 <- lm(techdebt ~ Workplace + Experience, data=d) 
summary(m1)
```  
### 5.1 Tesing Normality by using qqPlot and Shapiro-Wilk test.
...
```
### 5.2 Checking whether the model is in good quality by using the function of residualPlot().
...
### 5.3 Presenting the ANOVA table(one-way Anova).

```{r 5.3}
Anova(m1)
```

### 5.4 Using Tukey HSD post-hoc test to know the differences among groups.
